<?php include 'inc/header.php'; ?>
	<div class="clip-heading">
		<h1 style="color: #fff;">CLIPPING PATH</h1>
	</div>
	<div class="clip-area">
		<div class="clip-content container">
<?php 
	$query = "SELECT * FROM tbl_all_pages WHERE position='pageheader' && category='Clipping Path' order by id DESC limit 1 ";
...
<?php 
	$query = "SELECT * FROM tbl_all_pages WHERE category='Clipping Path' order by id DESC ";
	$position = $db->select($query);
	if ($position) {
		while ($result = $position->fetch_assoc()) {
?>
